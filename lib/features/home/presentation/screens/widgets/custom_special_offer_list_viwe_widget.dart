...
class CustomSpecialOfferListViewWidget extends StatefulWidget {
  final int categoryId;

  const CustomSpecialOfferListViewWidget({super.key, this.categoryId = 0});

  @override
  State<CustomSpecialOfferListViewWidget> createState() =>
      _CustomSpecialOfferListViewWidgetState();
}

class _CustomSpecialOfferListViewWidgetState
    extends State<CustomSpecialOfferListViewWidget> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<HomeCubit>().initSpecialOffers(_storeId, widget.categoryId);
    });
  }
  ...
  void didChangeDependencies() {
    ...
    if (currentStoreId != _storeId) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          context.read<HomeCubit>().initSpecialOffers(
            _storeId,
            widget.categoryId,
          );
        }
  ...
  void _onScroll() {
    ...
    if (reached) {
      context.read<HomeCubit>().loadMoreSpecialOffers(
        widget.categoryId,
        _storeId,
      );
    }
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<HomeCubit, HomeState>(
      builder: (context, state) {
        ...
        final status = state.getSpecialOffersStatusForCategory(
          categoryId,
          storeId: _storeId,
        );
        final items = state.getSpecialOffersForCategory(
          categoryId,
          storeId: _storeId,
        );
        final isLoading = state.isSpecialOffersLoadingForCategory(
          categoryId,
          storeId: _storeId,
        );
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              Text("special_offers".tr(), style: AppTextStyle.styleBlack18Bold),
