...
class CustomBestSellerListViewWidget extends StatefulWidget {
  final int categoryId;

  const CustomBestSellerListViewWidget({super.key, this.categoryId = 0});

  @override
  State<CustomBestSellerListViewWidget> createState() =>
      _CustomBestSellerListViewWidgetState();
}

class _CustomBestSellerListViewWidgetState
    extends State<CustomBestSellerListViewWidget> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<HomeCubit>().initBestSeller(_storeId, widget.categoryId);
    });
  }
  ...
  void _onScroll() {
    ...
    if (reached) {
      context.read<HomeCubit>().loadMoreBestSeller(widget.categoryId, _storeId);
    }
  }
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<HomeCubit, HomeState>(
      builder: (context, state) {
        ...
        final status = state.getBestSellersStatusForCategory(
          categoryId,
          storeId: _storeId,
        );
        final items = state.getBestSellersForCategory(
          categoryId,
          storeId: _storeId,
        );
        final isLoading = state.isBestSellersLoadingForCategory(
          categoryId,
          storeId: _storeId,
        );
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              Text("best_seller".tr(), style: AppTextStyle.styleBlack18Bold),
